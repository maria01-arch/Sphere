...
export async function POST(req) {
  // Whole handler wrapped — an uncaught exception anywhere here previously
  // meant Next.js's own generic error response reached the client instead
  // of a clean JSON error, which is why the alert just said "unknown error"
  // with no useful detail.
  try {
    // Fail fast and clearly on missing config, matching the precedent
    // already set in /api/delete-account for this exact env var.
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      return Response.json({ error: 'Server is not configured for AI mention replies (missing SUPABASE_SERVICE_ROLE_KEY)' }, { status: 500 })
    }
    if (!process.env.OPENROUTER_API_KEY) {
      return Response.json({ error: 'Server is not configured for AI mention replies (missing OPENROUTER_API_KEY)' }, { status: 500 })
    }
